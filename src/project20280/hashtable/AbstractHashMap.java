...
public abstract class AbstractHashMap<K, V> extends AbstractMap<K, V> {
  ...
  protected int capacity; // length of the table - number of buckets
  ...
  public V put(K key, V value) {
    V answer = bucketPut(hashValue(key), key, value); // first we compute the hash value (h that identifies the bucket,
                                                      // and then we put the new entry in there)
    if (n > capacity / 2) // we ensure that current bucket table capacity must be <= 0.5 of total capacity
      resize(2 * capacity - 1); // we resize it
    return answer;
  }
  ...
  private int hashValue(K key) {
    return (int) ((Math.abs(key.hashCode() * scale + shift) % prime) % capacity); // standard math magic
  }

  // OBSERVATION!!! - hashCode() is built in function that can convert string
  // inputs to key values (complicated algorithm)

  /* This is used because it tends to spread keys evenly between buckets */

  /**
   * Updates the size of the hash table and rehashes all entries.
   */
  private void resize(int newCap) {
    ArrayList<Entry<K, V>> buffer = new ArrayList<>(); // we must first remember all the entries
    for (Entry<K, V> e : entrySet())
      buffer.add(e);

    capacity = newCap; // update capacity
    createTable(); // create new table
    n = 0; // initialize size (number of total entries) to 0

    for (Entry<K, V> e : buffer)
      put(e.getKey(), e.getValue()); // add each entry
  }
